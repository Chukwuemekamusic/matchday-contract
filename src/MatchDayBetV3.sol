 ...
 *
 * V3 Changes:
 * - Made batch resolution idempotent (skips already-resolved matches instead of reverting)
 * - Added MatchResolutionSkipped event for observability
 * - Prevents entire batch from failing if one match is already resolved
 * - Added BatchMatchesResolvedSummary event for observability
    ...
    PausableUpgradeable
{
    ...
    // V3: Skip reasons for batch operations
    enum SkipReason {
        NONE,
    ...
    uint256 public constant MIN_GRACE_PERIOD = 105 minutes;
    uint256 public constant MAX_GRACE_PERIOD = 2 hours;
    ...
    // ============ V3 State Variables ============
    // NEW in V3 - Added after V2 variables

    /// @notice Grace period in seconds
    uint256 public gracePeriod;

    /// @dev Reserved storage gap for future upgrades (V3)
    uint256[40] private __gap_v3;
    ...
    modifier onlyMatchManager() {
        if (!matchManagers[msg.sender] && msg.sender != owner) {
            revert NotMatchManager();
        }
        _;
    }
    ...
    /**
     * @notice Initialize V3 features
     * @dev Called once after upgrade from V2 to V3
     */
    function initializeV3() public reinitializer(3) {
        // V3 initialization logic (if needed)
        // Currently no special initialization needed for V3
        // grace period  105 minutes i.e 90 mins + 15 mins half time
        if (gracePeriod == 0) {
    ...
    function createMatch(
        ...
        if (bytes(competition).length > MAX_STRING_LENGTH) {
    ...
    function batchResolveMatches(
        ...
        emit BatchMatchesResolvedSummary(matchIds, results, resolved, skipped);
    ...
    function cancelMatch(
        ...
        if (matchData.status == MatchStatus.CANCELLED) return;
    ...
    /**
     * @notice Set the grace period for match resolution
    ...
    function _tryResolveMatch(
        ...
        if (matchData.matchId == 0) {
            emit MatchResolutionSkipped(matchId, SkipReason.MATCH_NOT_FOUND);
            return false;
        }

        // Invalid outcome
        if (result == Outcome.NONE) {
            emit MatchResolutionSkipped(matchId, SkipReason.INVALID_OUTCOME);
            return false;
        }
        ...
        // Cancelled
        if (matchData.status == MatchStatus.CANCELLED) {
            emit MatchResolutionSkipped(matchId, SkipReason.MATCH_IS_CANCELLED);
            return false;
        }
    ...
    function _tryCancelMatch(
        ...
        // non-existent match
        if (matchData.matchId == 0) {
            emit MatchCancellationSkipped(matchId, SkipReason.MATCH_NOT_FOUND);
            return false;
        }
